...
@section('page-title', 'Laporan Jurnal: ' . $targetUser->name)

@section('content')
<a href="{{ route('admin.jurnal-scholarship-teenager.laporan') }}" class="btn btn-sm btn-link mb-2"><i class="fas fa-arrow-left"></i> Kembali</a>

<div class="card mb-3">
    <div class="card-body d-flex flex-wrap align-items-center">
        <div class="mr-auto">
            <h5 class="mb-0">{{ $targetUser->name }}</h5>
            <small class="text-muted">@ {{ $targetUser->username }} · {{ $targetUser->studentProfile?->grade_class ?? '—' }}</small>
        </div>
        <div>
...
<div class="card">
    <div class="card-header">
        <form method="GET" class="form-inline">
            ...
            <a href="{{ route('admin.jurnal-scholarship-teenager.export', ['user' => $targetUser->id, 'from' => $from->toDateString(), 'to' => $to->toDateString()]) }}"
