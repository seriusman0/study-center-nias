...
class MentorPresensiAdminController extends Controller
{
    public function index(Request $request)
    {
        ...
        $records = $q->latest('tanggal')->latest('jam_datang')->paginate(20)->withQueryString();

        $cabangs = $user->isAdmin() ? Cabang::orderBy('nama')->get() : collect();
        $mentors = $this->mentorList($user);

        return view('admin.mentor-presensi.index', compact('records', 'cabangs', 'mentors', 'from', 'to'));
    }
    ...
    public function reports(Request $request)
    {
        ...
        return view('admin.mentor-presensi.reports', array_merge($data, [
            'cabangs'   => $cabangs,
            'from'      => $from,
            'to'        => $to,
    ...
    public function exportExcel(Request $request)
    {
        ...
        return response()->streamDownload(function () use ($data, $from, $to) {
            ...
            fputcsv($out, ['Total Murid (sum)', $data['totals']['murid']]);
            fputcsv($out, ['Mentor Aktif', $data['totals']['mentor_aktif']]);
            fputcsv($out, []);

            fputcsv($out, ['Per Mentor']);
            fputcsv($out, ['Mentor', 'Sesi', 'Total Jam', 'Total Murid', 'Rata-rata Murid/Sesi']);
            foreach ($data['perMentor'] as $r) {
                fputcsv($out, [
                    $r->mentor?->name ?? '—',
                    $r->sesi,
                    round($r->menit_total / 60, 2),
                    $r->murid_total,
                    round((float) $r->murid_avg, 1),
                ]);
            }
            fputcsv($out, []);

            fputcsv($out, ['Per Cabang']);
            fputcsv($out, ['Cabang', 'Sesi', 'Total Murid']);
            foreach ($data['perCabang'] as $r) {
                fputcsv($out, [$r->cabang?->nama ?? '—', $r->sesi, $r->murid_total]);
            }
            fputcsv($out, []);
    ...
    public function exportPdf(Request $request)
    {
        ...
        if ($request->filled('cabang_id')) {
            $cabangNama = Cabang::where('id', $request->cabang_id)->value('nama');
        }

        $pdf = Pdf::loadView('admin.mentor-presensi.export-pdf', array_merge($data, [
            'from'        => $from,
            'to'          => $to,
    ...
    private function buildReportData(Request $request, Carbon $from, Carbon $to): array
    {
        ...
        if (! $user->isAdmin()) {
            if ($user->cabang_id) {
                $base->where('cabang_id', $user->cabang_id);
            } else {
                $base->whereRaw('1 = 0');
            }
    ...
    private function mentorList($user)
    {
        $mentorRoleId = Role::where('name', 'mentor')->value('id');
        if (! $mentorRoleId) return collect();
        $q = User::where('is_active', true)
            ->whereHas('roles', fn($r) => $r->where('roles.id', $mentorRoleId))
            ->orderBy('name');
