...
class JurnalReportController extends Controller
{
    ...
    public function show(Request $request, User $student)
    {
        ...
        if ($request->filled('from')) {
            ...
            // Default: dari tanggal entry pertama siswa ini, atau awal bulan jika tidak ada data
            $firstEntry = \App\Models\JurnalEntry::forStudent($student->id)
                ->orderBy('tanggal')->value(\Illuminate\Support\Facades\DB::raw('DATE(tanggal)'));
            $from = $firstEntry
    ...
    public function export(Request $request, User $student)
    {
        ...
        if ($request->filled('from')) {
            ...
            $firstEntry = \App\Models\JurnalEntry::forStudent($student->id)->orderBy('tanggal')->value('tanggal');
            $from = $firstEntry
                ? Carbon::parse($firstEntry, JurnalWeek::TZ)->startOfDay()
                : $today->copy()->startOfMonth();
        }
