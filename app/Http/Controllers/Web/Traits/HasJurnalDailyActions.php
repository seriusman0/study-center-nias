...
trait HasJurnalDailyActions
{
    ...
    {
        $viewer     = $request->user();
        $targetUser = User::where('is_active', true)->findOrFail($userId);

        abort_unless($targetUser->hasRole($this->role), 403);
        abort_if($targetUser->id === $viewer->id, 302, route($this->routePrefix . '.index'));
    ...
    public function toggle(Request $request)
    {
        ...
        DB::transaction(function () use ($user, $date, $data) {
            $type = $data['type'];
    ...
    public function uploadFoto(Request $request)
    {
        ...
        $path = $request->file('foto')->store('jurnal-foto/' . $date->format('Y/m'), 'public');
    ...
    private function loadStudyLogs(int $userId, Carbon $date, $itemIds)
    {
        return CollegeStudyLog::where('user_id', $userId)
            ->whereDate('tanggal', $date->toDateString())
            ->whereIn('life_item_id', $itemIds)
            ->get()
            ->keyBy('life_item_id');
    ...
    private function buildStudyState($studyLogs): array
    {
        return $studyLogs->mapWithKeys(fn($l) => [$l->life_item_id => [
    ...
    private function calcStreak(int $userId, Carbon $today): int
    {
        $streak = 0;
        $cursor = $today->copy();
        for ($i = 0; $i < 60; $i++) {
            $d   = $cursor->toDateString();
            $has = JurnalEntry::forStudent($userId)->whereDate('tanggal', $d)
                    ->where(fn($q) => $q->where('pl_checked', true)->orWhere('pb_checked', true))
                    ->exists()
                || JurnalLifeCheck::forStudent($userId)->whereDate('tanggal', $d)->where('checked', true)->exists();
