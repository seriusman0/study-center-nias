...
(async () => {
    ...
    // We need to login as college user. I'll just use testuser or check what user is college.
    // wait, what users are there? Let's check the database.
    await page.fill('input[name="login"]', 'testuser'); // I hope this is college role.
    await page.fill('input[name="password"]', '12345');
    await page.click('button[type="submit"]');
    await page.waitForLoadState('networkidle');

    await page.goto('http://localhost:8888/jurnal-college');
    await page.waitForLoadState('networkidle');
    ...
    if (!btnBelum || !btnSudah) {
        console.log('Buttons not found. Maybe user is not college role?');
    } else {
